...
class CNNLayerVisualization():
    ...
    def __init__(self, path_model, prefix, ngpu=1, if_cuda=True):
        ...
        self.model = models.subnet_face50(**model_options)
    ...
    def visualise_layer_with_hooks(self, img, selected_layer, selected_filter, demog_label=[3]):
        ...
        for i in range(1, 31):
            ...
            output = remakeImage(output)
            output = np.uint8(output)
            self.created_image = Image.fromarray(output, 'RGB')
            # Save image
            if i % 5 == 0:
    ...
    def visualise_layer_without_hooks(self, img, selected_layer, selected_filter, demog_label=[3]):
        self.selected_layer = selected_layer
        self.selected_filter = selected_filter
        demog_label = torch.LongTensor(demog_label)
        if self.if_cuda:
...
def main():
    ...
    savepath_pic = '/research/prip-gongsixu/codes/biasface/results/figures/neurIPS2020/visual/filter/gac/am/'
    if os.path.isdir(savepath_pic) is False:
        os.makedirs(savepath_pic)
    
    path_pic = '/research/prip-gongsixu/codes/biasface/results/figures/neurIPS2020/visual/am_sample.jpg'
