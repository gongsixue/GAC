    ...
    def forward(self, x, demog_label, epoch):
        ...
        if self.adap:
            for i,demog in enumerate(demogs):
                ...
                #     else:
                #         # kernel_mask = self.kernel_adap[demog,:,:,:,:]
                #         k_input = F.one_hot(torch.tensor([demog]), num_classes=self.ndemog)
                # else:
                #     # kernel_mask = self.kernel_adap[demog,:,:,:,:]
                #     k_input = F.one_hot(torch.tensor([demog]), num_classes=self.ndemog)
                # if x.is_cuda:
                ...
                ########### get output ################
                if i == 0:
                    temp = F.conv2d(x[indices,:,:,:], self.kernel_base*mask,
                        stride=self.stride, padding=self.padding)
                    # temp = self.conv(x[indices,:,:,:])
                    # initialize output
                    size = [x.size(0)]
            ...
            # if x.is_cuda:
            #     kernel_mask = kernel_mask.cuda()
            # output = F.conv2d(x, kernel_mask, stride=self.stride, padding=self.padding)
